...
%% Fraction Sarcomeric
  ...
  % Sarcomeric region for single peak 0.7 to 1.0 microns (bins 12 to 20), for actin 1.55 to 2.15 microns (bins 29 to 43). 
  % 
 
  sarcregion = zeros(size(bins, 2)-1, 1);
  sarcregion([12:20, 29:43],1) = 1;
...
%%
save([filein(1:end-4), '.mat'], 'bins', 'histosum', 'disthistoscaled','meansarcregion','meannotsarc', 'sarcfraction', 'meanGray', '-mat')
...
%% SarcFraction Binning
    ...
    MeanGrayScale = nan(length(filelist), 1);
...
%%
    ...
    results = table(processedFiles, MeanSarcomeric, NonSarcomericMean, SarcomericFraction, MeanGrayScale);
